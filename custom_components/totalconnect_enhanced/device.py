...
class TotalConnectDevice:
    ...
    def is_garage_door(self) -> bool:
        """Return True if this is a garage door opener."""
        return (
            self.class_id == 303  # MyQ Garage Door from DEVICES.md
            or self.class_id == 302  # Some door/garage devices (but not if name contains "door")
            or "garage" in self.name.lower()
            or "myq" in self.name.lower()
        ) and not (
            # Exclude devices that are likely smart locks
            "front door" in self.name.lower()
            or "back door" in self.name.lower()
        )

    def is_smart_lock(self) -> bool:
        """Return True if this is a smart lock."""
        return (
            "lock" in self.name.lower()
            and ("door" in self.name.lower() or "front" in self.name.lower() or "back" in self.name.lower())
        ) or (
            self.class_id == 7  # Skybell doorbell class, some locks may use this
            and "lock" in self.name.lower()
        ) or (
            self.class_id == 12  # Some door/lock devices use this class
            and "door" in self.name.upper()
        ) or (
            self.class_id == 302  # Smart locks often use this class
            and "door" in self.name.lower()
        )

    def get_automation_device_status(self) -> dict[str, Any]:
        """Get automation device status information."""
        return {
            "device_id": self.deviceid,
            "name": self.name,
            "class_id": self.class_id,
            "is_garage_door": self.is_garage_door(),
            "is_smart_lock": self.is_smart_lock(),
