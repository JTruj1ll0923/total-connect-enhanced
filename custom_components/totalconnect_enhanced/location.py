...
from .const import (
    HTTP_API_AUTOMATION_DEVICE_STATUS,
    HTTP_API_DEVICE_CONTROL,
    HTTP_API_GARAGE_DOORS,
    PROJECT_URL,
    SOAP_URL,
    SOAP_CONTROL_LOCK,
    SOAP_CONTROL_SWITCH,
...
from .exceptions import (
    AutomationDeviceControlFailed,
    AutomationDeviceNotFound,
    FailedToBypassZone,
...
class TotalConnectLocation:
    ...
    def __init__(self, location_info_basic: dict[str, Any], parent: "TotalConnectClient") -> None:
        ...
        self._module_flags: dict[str, str] = dict(
        ...
        self.last_updated_timestamp_ticks: int | None = None
        self.configuration_sequence_number: int | None = None
        self.arming_state: ArmingState = ArmingState.UNKNOWN
        self.partitions: dict[int, TotalConnectPartition] = {}
        self._partition_list: list[int] = []
        self.zones: dict[int, TotalConnectZone] = {}
        self.usercode: str = DEFAULT_USERCODE
        self.auto_bypass_low_battery: bool = False
        self._sync_job_id: str | None = None
    ...
    def get_garage_doors(self) -> dict[str, dict[str, Any]]:
        """Get all garage doors from REST API."""
        garage_doors = {}
        
        try:
            # Get garage doors from REST API
            garage_url = HTTP_API_GARAGE_DOORS.format(location_id=self.location_id)
            result = self.parent.http_request(garage_url, "GET")
            
            if result.get("ResultCode") == 0:
                wifigaragedoors = result.get("Wifigaragedoors", {})
                for garage in wifigaragedoors.get("garageDoors", []):
                    garage_doors[garage["id"]] = {
                        "id": garage["id"],
                        "name": garage["name"],
                        "state": garage["state"],
                        "online": garage["online"],
                        "switch_id": garage.get("switchConfiguration", {}).get("SwitchID"),
                        "gateway_id": garage.get("gateway_id"),
                        "can_remote_close": garage.get("can_remote_close"),
                        "battery_low": garage.get("battery_low"),
                    }
            
        except TotalConnectError as err:
            LOGGER.warning(f"Could not get garage doors: {err}")
        
        return garage_doors

    def get_smart_locks(self) -> dict[str, TotalConnectDevice]:
        """Get all smart locks from device list."""
        smart_locks = {}
        
        for device_id, device in self.devices.items():
            if device.is_smart_lock():
                smart_locks[device_id] = device
        
        return smart_locks

    def get_automation_device_status(self, device_id: str) -> dict[str, Any]:
        """Get status for a specific automation device."""
        if device_id not in self.devices:
            raise AutomationDeviceNotFound(f"Device {device_id} not found at location {self.location_id}")

        device = self.devices[device_id]

        # Try to get detailed automation status
        try:
            result = self.parent.http_request(
                endpoint=HTTP_API_AUTOMATION_DEVICE_STATUS,
                method="POST",
            ...
            LOGGER.warning(f"Could not get automation status for device {device_id}: {err}")
            # Return basic device info if automation status fails
            return device.get_automation_device_status()

    def control_garage_door(self, device_id: str, action: str) -> bool:
        """Control a garage door (open/close) using SOAP API."""

        # Get garage doors from REST API to find switch info
        garage_doors = self.get_garage_doors()
        
        if device_id not in garage_doors:
            raise AutomationDeviceNotFound(f"Garage door {device_id} not found at location {self.location_id}")
        
        garage_info = garage_doors[device_id]
        switch_id = garage_info.get("switch_id")
        
        if not switch_id:
            raise AutomationDeviceControlFailed(f"Garage door {device_id} has no switch ID")
        
        # Find automation device
        automation_device = None
        for device in self.devices.values():
            if device.name == "Automation":
                automation_device = device
                break
        
        if not automation_device:
            raise AutomationDeviceControlFailed("Automation device not found")
        
        # Map action to switch state
        action_map = {
            "open": 1,   # Open garage door
            "close": 0,  # Close garage door
        }
        
        if action not in action_map:
            raise AutomationDeviceControlFailed(f"Invalid action '{action}'. Use 'open' or 'close'")
        
        switch_action = action_map[action]
        
        # Create SOAP request
        soap_data = f"""<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <ControlASwitch xmlns="https://services.alarmnet.com/TC2/">
      <SessionID>{self.parent.sessionId}</SessionID>
      <DeviceID>{automation_device.deviceid}</DeviceID>
      <SwitchID>{switch_id}</SwitchID>
      <SwitchAction>{switch_action}</SwitchAction>
    </ControlASwitch>
  </soap:Body>
</soap:Envelope>"""
        ...
        headers = {
            ...
            "SOAPAction": SOAP_CONTROL_SWITCH,
        }
        
        try:
            import requests
            response = requests.post(SOAP_URL, data=soap_data, headers=headers)
            
            if response.status_code != 200:
                raise AutomationDeviceControlFailed(f"HTTP {response.status_code}: {response.text}")
            
            # Check SOAP response
            if "ResultCode>0<" in response.text:
                LOGGER.info(f"Garage door {device_id} {action} command sent successfully")
                return True
            else:
                raise AutomationDeviceControlFailed(f"SOAP error: {response.text}")
                
        except requests.RequestException as err:
            error_msg = f"Failed to {action} garage door {device_id}: {err}"
            LOGGER.error(error_msg)
            raise AutomationDeviceControlFailed(error_msg) from err

    def control_smart_lock(self, device_id: str, action: str) -> bool:
        """Control a smart lock (lock/unlock) using SOAP API."""

        if device_id not in self.devices:
            raise AutomationDeviceNotFound(f"Smart lock {device_id} not found at location {self.location_id}")
        
        device = self.devices[device_id]
        if not device.is_smart_lock():
            raise AutomationDeviceControlFailed(f"Device {device_id} is not a smart lock")
        
        # Find automation device
        automation_device = None
        for dev in self.devices.values():
            if dev.name == "Automation":
                automation_device = dev
                break
        
        if not automation_device:
            raise AutomationDeviceControlFailed("Automation device not found")
        
        # Map action to lock state
        action_map = {
            "lock": 1,     # Lock
            "unlock": 0,   # Unlock
        }
        ...
        if action not in action_map:
            raise AutomationDeviceControlFailed(f"Invalid action '{action}'. Use 'lock' or 'unlock'")
        
        lock_state = action_map[action]
        
        # Create SOAP request
        soap_data = f"""<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <ControlALock xmlns="https://services.alarmnet.com/TC2/">
      <SessionID>{self.parent.sessionId}</SessionID>
      <DeviceID>{automation_device.deviceid}</DeviceID>
      <LockID>{device_id}</LockID>
      <LockDesiredState>{lock_state}</LockDesiredState>
      <AuthorizingCode>0000</AuthorizingCode>
    </ControlALock>
  </soap:Body>
</soap:Envelope>"""
        ...
        try:
            ...
            # Check SOAP response
            if "ResultCode>0<" in response.text:
                LOGGER.info(f"Smart lock {device_id} {action} command sent successfully")
                return True
            else:
                raise AutomationDeviceControlFailed(f"SOAP error: {response.text}")
                
        except requests.RequestException as err:
            error_msg = f"Failed to {action} smart lock {device_id}: {err}"
            LOGGER.error(error_msg)
            raise AutomationDeviceControlFailed(error_msg) from err
