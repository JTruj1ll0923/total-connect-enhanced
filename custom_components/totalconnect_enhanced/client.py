...
class TotalConnectClient:
    ...
    def raise_for_resultcode(self, response: dict[str, Any]) -> None:
        ...
        if rc == _ResultCode.AUTOMATION_DEVICE_NOT_FOUND:
            raise AutomationDeviceNotFound(rc.name, response)
        if rc == _ResultCode.AUTOMATION_DEVICE_OFFLINE:
            raise AutomationDeviceOffline(rc.name, response)
